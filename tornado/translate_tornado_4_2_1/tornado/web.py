...
###
``tornado.web``提供了一种简单的Web框架与异步功能,使其能够扩展以大量的开放式连接,
使它非常适合`长轮询`。
其中定义了 Application 和 RequestHandler 类，在 app.py 里直接就用到了。
Application 是个单例，总揽全局路由，创建服务器负责监听，并把服务器传回来的请求进行转发（__call__）。
RequestHandler 是个功能很丰富的类，基本上 web 开发需要的它都具备了，
比如redirect，flush，close，header，cookie，render（模板），xsrf，etag等等
###
...
request.
###########################################
总的说，在tornado中'RequestHandler'方法和其他地方不是多线程安全的。
特别是,这种方法为"~RequestHandler.Write()"、"~RequestHandler.finish()"、
"~RequestHandler.flush()"必须是从主线程中被调用。
如果您使用多线程的重要的是要使用“IOLoop.add_callback”传送控制权返回给主线程结束前的请求。
