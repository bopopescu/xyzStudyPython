...
for i in range(16):
    ...
    B_list.append(
        Button(frame2, width=4, height=2, textvariable=str_data[i], font=("Courier 20 bold roman"), bg=dic_color[0]))
...
def CanMove(derection, index):
    for i in range(1, 4):
        if list_data[derection_index[derection][index][i]] == list_data[derection_index[derection][index][i - 1]]:
            return True
        if list_data[derection_index[derection][index][i]] != 0 and list_data[
            derection_index[derection][index][i - 1]] == 0:
            return True
    return False
